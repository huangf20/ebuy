	...
	<form action="userServlet?oper=userSave" method="post" onsubmit="return checkForm()">
		<table class="form">
			...
			<tr>
				...
				<td><input type="text" class="text"  id="trueName1" name="trueName" value="${user.trueName }" /></td>
			</tr>
			<tr>
				...
				<td>
					<c:choose>
						<c:when test="${user.sex=='男' }">
							<input type="radio" name="sex"  value="男" checked="checked" />男 <input type="radio" name="user.sex" value="女" />女
						</c:when>
						<c:otherwise>
							<input type="radio" name="sex"  value="男"  />男 <input type="radio" name="user.sex" value="女"  checked="checked"/>女
						</c:otherwise>
					</c:choose>
				</td>
			</tr>
