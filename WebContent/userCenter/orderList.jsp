...
function confirmReceive(orderId){
	if(confirm("确认收货？")){
		$.post("orderServlet?oper=status",{orderId:orderId},function(){
...
<h2>订单管理</h2>
		<div class="manage">
			<div class="search">
				<form action="orderServlet?oper=search" method="post">
					订单号：<input type="text" class="text" name="orderNo" value="${s_order.orderNo}" /> 
			...
			<div class="spacer"></div>
				...
				<tr style="background-color: #f7f4eb">
					...
					<c:choose>
						...
						<c:when test="${order.status==3 }"><input type="button" style="border: 1px solid red" value="确实收货" onclick="confirmReceive(${order.id})"/></c:when>
